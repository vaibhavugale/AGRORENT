  ...
  try {
    ...
    if (newEquipment) {
      try {
        ...
        const userEqu = await User.findById({ _id: userId }).populate(
          "equipments"
        );
...
exports.unBookEquipment = async (req, res) => {
  try {
    const { eupId, hisId } = req.body || {};
    console.log("EUPID", eupId);

    if (!eupId)
      return res.status(404).json({
        success: false,
        message: "eup id not found",
      });
